...
\newcommand{\posterabstract}[2]{%
  ...
  \begin{minipage}{\linewidth}%
    \label{#1}%
          {\bfseries\normalsize\papertitle{#1}}  \hfill {#2}\\
  ...
  \begin{minipage}{\linewidth}%
    \label{#5}%
      %\rule{\linewidth}{1pt}\linebreak
          {\bfseries\normalsize\papertitle{#5}} \hfill {#6} \\
          \hfill\normalsize\paperauthor{\citefullauthornames{#5}}
%      {\small #1 #2 --- #4}\vspace{.5ex}\\
        % ACL2020: no specific time slot for each paper. The whole session can be used for QA
          \hfill {\small #2}
...
%    {\huge{\Large Workshop #2:}\vspace{.5ex}\\
    ...
    {\large \href{https://virtual.acl2020.org/workshop_W#2.html}{[Virtual Portal Website]} \href{#5}{[Workshop Website]}}\\
    \vspace{2mm}{\small The schedule here may not be up-to-date, please check the workshop website for the latest one.}
...
\newenvironment{tutorial}[5]{%
  ...
  \section[\textbf{T\theTutorialCounter:} {\papertitle{#2}}]{Tutorial \theTutorialCounter}
  \label{tutorial-#2}
  \begin{center}
    \begin{Large}
...
\newenvironment{SessionOverview}[9]{
  \section[#1]{#1 Overview -- #2}
  \setheaders{#1}{#2}
  \begin{center}
    \righthyphenmin2
    \sloppy
    \begin{tabular}{>{\RaggedRight}p{0.69in}|>{\RaggedRight}p{0.69in}|>{\RaggedRight}p{0.69in}|>{\RaggedRight}p{0.69in}|>{\RaggedRight}p{0.69in}|>{\RaggedRight}p{0.69in}|>{\RaggedRight}p{0.69in}}
      \bf Track A & \bf Track B & \bf Track C & \bf Track D & \bf Track E & \bf Track F & \bf Track G \\
      \it #3 & \it #4 & \it #5 & \it #6 & \it #7 & \it #8 & \it #9 \\
      \TrackALoc & \TrackBLoc & \TrackCLoc & \TrackDLoc & \TrackELoc & \TrackFLoc & \TrackGLoc \\
      \hline\hline}
